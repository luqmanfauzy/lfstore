                                                ...
                                                @foreach ($getImageEditorActions(iconSizeClasses: 'h-5 w-5 mx-auto') as $groupedActions)
                                                    ...
                                                    >
                                                        @foreach ($groupedActions as $action)
                                                            <x-filament::button
                                                                x-tooltip="{ content: {{ json_encode($action['label']) }}, theme: $store.theme }"
                                                    ...
                                                    @foreach (collect($aspectRatios)->chunk(5) as $ratiosChunk)
                                                        ...
                                                        >
                                                            @foreach ($ratiosChunk as $label => $ratio)
                                                                <x-filament::button
                                                                    x-tooltip="{ content: {{ json_encode(__('filament-forms::components.file_upload.editor.actions.set_aspect_ratio.label', ['ratio' => $label])) }}, theme: $store.theme }"
