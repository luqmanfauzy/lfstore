...
<body id="home">
    ...
    <div class="container">
        ...
        <!-- Existing Products Loop -->
        <div class="row g-4">
            @foreach ($produkUnggulan as $data)
            <div class="col-6 col-md-4 col-lg-3">
                <div class="product-card">
                    <div class="product-image-wrapper">
                        <img src="{{ asset($data->image_thumbnail) }}" alt="{{ $data->name }}" class="product-image">
                        @if($data->discount)
                            <span class="product-badge">-{{ $data->discount }}%</span>
                        @endif
                    </div>
                    <div class="product-info">
                        <h3 class="product-name">{{ $data->name }}</h3>
                        <div class="product-price">Rp{{ number_format($data->price, 0, ',', '.') }}</div>
                        <a href="{{ route('detail', $data->slug) }}" class="product-btn text-decoration-none">
