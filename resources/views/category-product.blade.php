        ...
        <div class="flex flex-col md:flex-row gap-8 lg:gap-12">
                    ...
                    <ul class="space-y-2">
                        <li>
                            <a href="{{ route('catalog') }}"
                                class="{{ url()->current() == route('catalog') ? 'bg-blue-50 text-blue-600 font-semibold border-l-4 border-blue-600 pl-3' : 'text-gray-500 hover:text-gray-900 hover:bg-gray-50 border-l-4 border-transparent pl-3' }} flex items-center py-2 transition-all text-sm rounded-r-lg">
                                Semua Produk
                            </a>
                        </li>
                        @foreach ($categories as $category)
            ...
            <main class="w-full">
                ...
                <div class="md:hidden flex overflow-x-auto gap-3 pb-2 mb-6 scrollbar-hide -mx-4 px-4">
                    <a href="{{ route('catalog') }}"
                        class="{{ url()->current() == route('catalog') ? 'bg-blue-600 text-white border-blue-600' : 'bg-white border-gray-200 text-gray-700 hover:border-gray-300' }} border px-5 py-2 rounded-full text-sm font-medium whitespace-nowrap transition-colors shadow-sm">
                        Semua
                    </a>
                    @foreach ($categories as $category)
                        <a href="{{ route('category-product', $category->slug) }}"
                ...
                <div class="mb-8">
                    ...
                    <div class="mb-6 max-w-xl">
                        <form action="{{ route('category-product', ['slug' => request()->route('slug')]) }}"
                            method="GET" class="relative flex items-center">
                    ...
                    <div
                        ...
                        <h1 class="text-2xl font-bold text-gray-900 tracking-tight">
                            Menampilkan: <span class="text-blue-600">{{ $categoryName }}</span>
                            @if(request('query'))
                    ...
                    <div class="grid grid-cols-2 lg:grid-cols-3 xl:grid-cols-3 gap-5 sm:gap-6 lg:gap-8">
                        @foreach ($allProductbyCategory as $data)
                            <div
                                ...
                                @php
                                    $imgPath = $data->image_thumbnail ? 'storage/' . $data->image_thumbnail : null;
                                ...
                                <a href="{{ route('detail', $data->slug) }}"
                                    ...
                                    @if($imgExists)
                                        <img src="{{ asset($imgPath) }}" alt="{{ $data->name }}"
                                            class="object-cover w-full h-full rounded-2xl group-hover:scale-105 transition-transform duration-700 ease-in-out">
                                    @else
                ...
                @else
                    ...
                    <div
                        ...
                        <div
                            class="w-16 h-16 sm:w-20 sm:h-20 bg-white rounded-full flex items-center justify-center mb-6 shadow-sm text-gray-400">
                        ...
                        <p class="text-gray-500 max-w-sm mx-auto mb-8 text-sm">Maaf, kami belum memiliki produk di kategori
